FUNCTION(TBTK_MESSAGE msg)
	IF(NOT TBTK_QUIET)
		MESSAGE(${msg})
	ENDIF(NOT TBTK_QUIET)
ENDFUNCTION(TBTK_MESSAGE)

#Print the path to the config file (Useful for finding which path that is
#actually used when debugging)
TBTK_MESSAGE("Using config file: ${CMAKE_CURRENT_LIST_DIR}/TBTKConfig.cmake")

#Include TBTKTargets.cmake
INCLUDE("${CMAKE_CURRENT_LIST_DIR}/TBTKTargets.cmake")

#Add custom Find-modules
LIST(APPEND CMAKE_MODULE_PATH "${CMAKE_CURRENT_LIST_DIR}/Modules/")

#Include TBTKVersionInfo.cmake
INCLUDE(TBTKVersionInfo)
INCLUDE(TBTKBuildOptions)

#Add link directory for global install
LINK_DIRECTORIES(${CMAKE_CURRENT_LIST_DIR}/..)
#Add link directory for build directory
#LINK_DIRECTORIES(${CMAKE_CURRENT_LIST_DIR}/../Lib/src/)

#Detect packages
FIND_PACKAGE(BLAS QUIET) #TODO put back required
FIND_PACKAGE(LAPACK QUIET) #TODO put back required
FIND_PACKAGE(HDF5 COMPONENTS C CXX QUIET)
FIND_PACKAGE(ARPACK QUIET)
FIND_PACKAGE(CUDA QUIET)
FIND_PACKAGE(CURL QUIET)
FIND_PACKAGE(Eigen3 QUIET)
FIND_PACKAGE(FFTW3 QUIET)
FIND_PACKAGE(GMP QUIET)
FIND_PACKAGE(Gnuplot QUIET)
FIND_PACKAGE(OpenCV QUIET)
IF(NOT TBTK_NO_OPEN_BLAS)
	FIND_PACKAGE(OpenBLAS QUIET)
ENDIF(NOT TBTK_NO_OPEN_BLAS)
FIND_PACKAGE(OpenMP QUIET)
FIND_PACKAGE(Python QUIET)
FIND_PACKAGE(SuperLU QUIET)
FIND_PACKAGE(wxWidgets QUIET)
FIND_PACKAGE(HIP QUIET)

FIND_PACKAGE(PkgConfig REQUIRED)
IF(PKG_CONFIG_FOUND)
	PKG_CHECK_MODULES(LIBINT2 libint2)
ENDIF(PKG_CONFIG_FOUND)

#Add libraries
#LIST(APPEND TBTK_LIBRARIES "-lTBTK")
LIST(APPEND TBTK_LIBRARIES "TBTK")

IF(ARPACK_FOUND)
	TBTK_MESSAGE("[X] ARPACK")
#	LIST(APPEND TBTK_LIBRARIES " -larpack")
	LIST(APPEND TBTK_LIBRARIES ${ARPACK_LIBRARY})
ELSE(ARPACK_FOUND)
	TBTK_MESSAGE("[ ] ARPACK")
ENDIF(ARPACK_FOUND)

IF(CUDA_FOUND)
	TBTK_MESSAGE("[X] CUDA")
	LIST(APPEND TBTK_LIBRARIES ${CUDA_LIBRARIES})
	LIST(APPEND TBTK_LIBRARIES ${CUDA_cusparse_LIBRARY})
	LIST(APPEND TBTK_LIBRARIES ${CUDA_cusolver_LIBRARY})
	ADD_DEFINITIONS(-DTBTK_CUDA_ENABLED)
ELSE(CUDA_FOUND)
	TBTK_MESSAGE("[ ] CUDA")
ENDIF(CUDA_FOUND)

IF(HIP_FOUND)
	MESSAGE("[X] HIP")
	# FIND_PACKAGE(ROCSOLVER QUIET REQUIRED) #TODO check if it still works without those packages
	FIND_PACKAGE(HIPSOLVER QUIET REQUIRED)
	FIND_PACKAGE(HIPSPARSE QUIET REQUIRED)
	# FIND_PACKAGE(ROCBLAS QUIET REQUIRED)
	# FIND_PACKAGE(ROCSPARSE QUIET REQUIRED)
	LIST(APPEND TBTK_LIBRARIES ${HIP_LIBRARIES})
	# LIST(APPEND TBTK_LIBRARIES ${ROCSOLVER_LIBRARIES})
	LIST(APPEND TBTK_LIBRARIES ${HIPSOLVER_LIBRARIES})
	LIST(APPEND TBTK_LIBRARIES ${HIPSPARSE_LIBRARIES})
	# LIST(APPEND TBTK_LIBRARIES ${ROCBLAS_LIBRARIES})
	LIST(APPEND TBTK_LIBRARIES hip::device)
	INCLUDE_DIRECTORIES(${HIPSOLVER_INCLUDE_DIRS})
	INCLUDE_DIRECTORIES(${HIP_INCLUDE_DIRS})
	INCLUDE_DIRECTORIES(${HIPSPARSE_INCLUDE_DIRS})
	# INCLUDE_DIRECTORIES(${ROCSOLVER_INCLUDE_DIRS})
	# INCLUDE_DIRECTORIES(${ROCBLAS_INCLUDE_DIRS})
	# INCLUDE_DIRECTORIES(${ROCSPARSE_INCLUDE_DIRS})
	ADD_DEFINITIONS(-DTBTK_HIP_ENABLED)
ELSE(HIP_FOUND)
	MESSAGE("[ ] HIP")
ENDIF(HIP_FOUND)

IF(CURL_FOUND)
	TBTK_MESSAGE("[X] cURL")
#	SET(TBTK_LIBRARIES "${TBTK_LIBRARIES} -lcurl")
	LIST(APPEND TBTK_LIBRARIES "curl")
ELSE(CURL_FOUND)
	TBTK_MESSAGE("[ ] cURL")
ENDIF(CURL_FOUND)

IF(EIGEN3_FOUND)
	TBTK_MESSAGE("[X] Eigen3")
	INCLUDE_DIRECTORIES(${EIGEN3_INCLUDE_DIRS})
ELSE(EIGEN3_FOUND)
	TBTK_MESSAGE("[ ] Eigen3")
ENDIF(EIGEN3_FOUND)

IF(FFTW3_FOUND)
	TBTK_MESSAGE("[X] FFTW3")
#	SET(TBTK_LIBRARIES "${TBTK_LIBRARIES} -lfftw3")
	LIST(APPEND TBTK_LIBRARIES "fftw3")
ELSE(FFTW3_FOUND)
	TBTK_MESSAGE("[ ] FFTW3")
ENDIF(FFTW3_FOUND)

IF(GMP_FOUND)
	TBTK_MESSAGE("[X] GMP")
	LIST(APPEND TBTK_LIBRARIES "gmp")
	LIST(APPEND TBTK_LIBRARIES "gmpxx")
	INCLUDE_DIRECTORIES(${GMP_INCLUDES})
ELSE(GMP_FOUND)
	TBTK_MESSAGE("[ ] GMP")
ENDIF(GMP_FOUND)

IF(TBTK_ENABLE_ALL_OPTIONS)
	IF(GNUPLOT_FOUND)
		TBTK_MESSAGE("[X] Gnuplot")
	ELSE(GNUPLOT_FOUND)
		TBTK_MESSAGE("[ ] Gnuplot")
	ENDIF(GNUPLOT_FOUND)
ENDIF(TBTK_ENABLE_ALL_OPTIONS)

IF(HDF5_FOUND)
	TBTK_MESSAGE("[X] HDF5")
#	LIST(APPEND TBTK_LIBRARIES "hdf5")
#	LIST(APPEND TBTK_LIBRARIES "hdf5_cpp")
	INCLUDE_DIRECTORIES(${HDF5_INCLUDE_DIRS})
#	LINK_DIRECTORIES(${HDF5_LIBRARY_DIRS})
	ADD_DEFINITIONS(${HDF_DEFINITIONS})
	LIST(APPEND TBTK_LIBRARIES ${HDF5_LIBRARIES})
ELSE(HDF5_FOUND)
	TBTK_MESSAGE("[ ] HDF5")
ENDIF(HDF5_FOUND)

IF(TBTK_ENABLE_ALL_OPTIONS)
	IF(LIBINT2_FOUND)
		TBTK_MESSAGE("[X] Libint2")
		INCLUDE_DIRECTORIES(${LIBINT2_INCLUDES})
		LIST(APPEND TBTK_LIBRARIES ${LIBINT2_LIBRARIES})
	ELSE(LIBINT2_FOUND)
		TBTK_MESSAGE("[ ] Libint2")
	ENDIF(LIBINT2_FOUND)
ENDIF(TBTK_ENABLE_ALL_OPTIONS)

IF(OpenCV_FOUND)
	TBTK_MESSAGE("[X] OpenCV")
#	SET(TBTK_LIBRARIES "${TBTK_LIBRARIES} ${OpenCV_LIBRARIES}")
	LIST(APPEND TBTK_LIBRARIES ${OpenCV_LIBRARIES})
ELSE(OpenCV_FOUND)
	TBTK_MESSAGE("[ ] OpenCV")
ENDIF(OpenCV_FOUND)

IF(TBTK_ENABLE_ALL_OPTIONS)
	IF(OpenBLAS_FOUND)
		TBTK_MESSAGE("[X] OpenBLAS")
		SET(TBTK_USE_OPEN_BLAS TRUE)
	ELSE(OpenBLAS_FOUND)
		TBTK_MESSAGE("[ ] OpenBLAS")
	ENDIF(OpenBLAS_FOUND)
ENDIF(TBTK_ENABLE_ALL_OPTIONS)

IF(OpenMP_FOUND OR OPENMP_FOUND)
	TBTK_MESSAGE("[X] OpenMP")
	SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${OpenMP_CXX_FLAGS}")
	ADD_DEFINITIONS(-DTBTK_USE_OPEN_MP)
ELSE(OpenMP_FOUND OR OPENMP_FOUND)
	TBTK_MESSAGE("[ ] OpenMP")
ENDIF(OpenMP_FOUND OR OPENMP_FOUND)

IF(Python_FOUND)
	TBTK_MESSAGE("[X] Python " ${TBTK_PYTHON_VERSION})
	IF(TBTK_PYTHON_COMPILED)
		MESSAGE("	[X] Python ${TBTK_PYTHON_VERSION}")
	ELSE(TBTK_PYTHON_COMPILED)
		MESSAGE("	[ ] Python")
	ENDIF(TBTK_PYTHON_COMPILED)
	IF(TBTK_PYTHON_NUMPY_COMPILED)
		MESSAGE("	[X] Numpy (C/C++ development library)")
	ELSE(TBTK_PYTHON_NUMPY_COMPILED)
		MESSAGE("	[ ] Numpy (C/C++ development library)")
	ENDIF(TBTK_PYTHON_NUMPY_COMPILED)
	IF(TBTK_PYTHON_MATPLOTLIB_COMPILED)
		MESSAGE("	[X] Matplotlib")
	ELSE(TBTK_PYTHON_MATPLOTLIB_COMPILED)
		MESSAGE("	[ ] Matplotlib")
	ENDIF(TBTK_PYTHON_MATPLOTLIB_COMPILED)
	LIST(APPEND TBTK_LIBRARIES ${PYTHON_LIBRARIES})
	INCLUDE_DIRECTORIES(${PYTHON_INCLUDES})
ELSE(Python_FOUND)
	TBTK_MESSAGE("[ ] Python (with matplotlib and numpy)")
	IF(TBTK_PYTHON_COMPILED)
		MESSAGE("	[X] Python ${TBTK_PYTHON_VERSION}")
	ELSE(TBTK_PYTHON_COMPILED)
		MESSAGE("	[ ] Python")
	ENDIF(TBTK_PYTHON_COMPILED)
	IF(TBTK_PYTHON_NUMPY_COMPILED)
		MESSAGE("	[X] Numpy (C/C++ development library)")
	ELSE(TBTK_PYTHON_NUMPY_COMPILED)
		MESSAGE("	[ ] Numpy (C/C++ development library)")
	ENDIF(TBTK_PYTHON_NUMPY_COMPILED)
	IF(TBTK_PYTHON_MATPLOTLIB_COMPILED)
		MESSAGE("	[X] Matplotlib")
	ELSE(TBTK_PYTHON_MATPLOTLIB_COMPILED)
		MESSAGE("	[ ] Matplotlib")
	ENDIF(TBTK_PYTHON_MATPLOTLIB_COMPILED)
ENDIF(Python_FOUND)

IF(SuperLU_FOUND)
	TBTK_MESSAGE("[X] SuperLU")
	LIST(APPEND TBTK_LIBRARIES ${SUPER_LU_LIBRARIES})
	INCLUDE_DIRECTORIES(${SUPER_LU_INCLUDES})
ELSE(SuperLU_FOUND)
	TBTK_MESSAGE("[ ] SuperLU")
ENDIF(SuperLU_FOUND)

IF(TBTK_ENABLE_ALL_OPTIONS)
	IF(wxWidgets_FOUND)
		TBTK_MESSAGE("[X] wxWidgets")
		INCLUDE(${wxWidgets_USE_FILE})
		SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${wxWidgets_CXX_FLAGS}")
		LIST(APPEND TBTK_LIBRARIES ${wxWidgets_LIBRARIES})
		LINK_DIRECTORIES(${wxWidgets_LIBRARY_DIRS})
	ELSE(wxWidgets_FOUND)
		TBTK_MESSAGE("[ ] wxWidgets")
	ENDIF(wxWidgets_FOUND)
ENDIF(TBTK_ENABLE_ALL_OPTIONS)

#LIST(APPEND TBTK_LIBRARIES "-lhdf5 -lhdf5_cpp -llapack -lblas")
# LIST(APPEND TBTK_LIBRARIES "lapack") #TODO later
# IF(TBTK_USE_OPEN_BLAS)
# 	LIST(APPEND TBTK_LIBRARIES "openblas")
# ELSE(TBTK_USE_OPEN_BLAS)
# 	LIST(APPEND TBTK_LIBRARIES "blas")
# ENDIF(TBTK_USE_OPEN_BLAS)
