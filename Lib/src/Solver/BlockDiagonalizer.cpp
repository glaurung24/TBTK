	...
	namespace Solver
	{
		...
		void BlockDiagonalizer::init()
		{
				...
				n++)
			{
				...
				else
				{
					...
					eigenValuesOffsets.push_back(
						eigenValuesOffsets.back() + blockStructureDescriptor.getNumStatesInBlock(n - 1));
				}
			}
			...
			for (
				...
				n++)
			{
				size_t numStatesInBlock = blockStructureDescriptor.getNumStatesInBlock(n);
			...
			if (!useGPUAcceleration)
			{ // No hamiltonian needs to be stored as GPU solver
		...
		void BlockDiagonalizer::update()
		{
			...
			for (
				...
				n++)
			{
				unsigned int numStatesInBlock = blockStructureDescriptor.getNumStatesInBlock(n);
				if (!useGPUAcceleration)
				{ // TODO See if one could avoid this
					hamiltonianSize += (numStatesInBlock * (numStatesInBlock + 1)) / 2;
				}
				else
				{
...
#pragma omp parallel for
				for (unsigned int block = 0; block < iterators.size(); block++)
				{
					...
					while (iterator != endIterator)
					{
						...
						if (from <= to)
						{
							unsigned int numStates = blockStructureDescriptor.getNumStatesInBlock(block);
							if (useGPUAcceleration)
							{
								// Temporarily store hamiltonian in eigenVectors
								eigenVectors[blockOffsets.at(block) +
											 to +
											 from * numStates] += (*iterator).getAmplitude();
							}
							else
							{
								hamiltonian[blockOffsets.at(block) + to + (from * (2 * numStates - from - 1)) / 2] += (*iterator).getAmplitude();
							}
						}
						++iterator;
					}
			...
			else
			{
				...
				while (blockIterator != blockIndices.cend())
				{
					...
					HoppingAmplitudeSet::ConstIterator iterator = getModel().getHoppingAmplitudeSet().cbegin(
						blockIndex);
					int minBasisIndex = iterator.getMinBasisIndex();
					unsigned int numStates = blockStructureDescriptor.getNumStatesInBlock(blockCounter);
					while (
						iterator != getModel().getHoppingAmplitudeSet().cend(blockIndex))
					{
						int from = model.getHoppingAmplitudeSet().getBasisIndex(
									   (*iterator).getFromIndex()) -
								   minBasisIndex;
						...
						if (from <= to)
						{
							...
							{
								hamiltonian[blockOffsets.at(blockCounter) + to + (from * (2 * numStates - from - 1)) / 2] += (*iterator).getAmplitude();
							}
						}
						++iterator;
					}
		...
		{
			unsigned num_blocks = blockStructureDescriptor.getNumBlocks();
			vector<bool> solvedBlocks(num_blocks, false);
			if (parallelExecution)
			{
				#pragma omp parallel for
				for (
					...
					b++)
				{
					if (useGPUAcceleration)
					{
						...
						catch (OutOfDeviceMemoryException &exception)
						{
							Streams::err << "Warning out of device memory. Retrying with serial calculation" << endl;
							solvedBlocks[b] = false;
						}
					}
					else
					{
						solveCPU(hamiltonian.getData() + blockOffsets.at(b),
									eigenValues.getData() + eigenValuesOffsets[b],
			...
			//If one of the parallel excecution has failed, the solver will retry the calculation again in this loop
			// Otherwise, every loop step will be skipped
			for (
				unsigned int b = 0;
				b < num_blocks;
				b++)
			...
			const Index &index)
		{
			...
			if (
				linearIndex >= firstStateInBlock && linearIndex <= lastStateInBlock)
			{
			...
			const Index &intraBlockIndex)
		{
			TBTKAssert(eigenVectorsAvailable == true,
					   ...
					   "Eigenvectors not available.",
					   "Use CalculationMode::EigenValuesAndEigenVectors instead.");
			int firstStateInBlock = getModel().getHoppingAmplitudeSet().getFirstIndexInBlock(blockIndex);
			unsigned int block = blockStructureDescriptor.getBlockIndex(
				firstStateInBlock);
			TBTKAssert(
				state >= 0 && state < (int)blockStructureDescriptor.getNumStatesInBlock(
										  block),
				"BlockDiagonalizer::getAmplitude()",
